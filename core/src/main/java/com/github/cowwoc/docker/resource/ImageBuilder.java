...
import com.github.cowwoc.docker.internal.buildkit.Build;
import com.github.cowwoc.docker.internal.buildkit.Session;
 ...
 * Builds an image.
 */
@SuppressWarnings({"PMD.UnusedPrivateField", "PMD.UnusedLocalVariable", "PMD.AvoidPrintStackTrace", "PMD.UnusedPrivateMethod"})
...
//		Path absoluteBuildContext = buildContext.toAbsolutePath().normalize();
		if (dockerfile == null)
		{
			dockerfile = buildContext.resolve("Dockerfile");
			if (Files.notExists(dockerfile))
			{
				// https://github.com/moby/moby/pull/10858
				Path lowercase = buildContext.resolve("dockerfile");
...
//		Path dockerfileAsPath = absoluteBuildContext.relativize(this.dockerfile.toAbsolutePath().normalize());
		...
		// Use Dockerfile Explorer to translate Dockerfiles into LLB operations that need to be sent to BuildKit
		// Based on https://github.com/moby/buildkit?tab=readme-ov-file#exploring-dockerfiles
		try (BuildKitConnection connection = client.getBuildKitConnection().get())
		{
			// Initially, buildx invokes control.build with a nil definition, session and "" ref.
			// solve() translates a "" ref into a random new ID and creates a new session.
			// It then runs the following in parallel:
			//
			// 1. control.status() processes all elements until EOS and then shuts down.
			// 2. Invokes gateway.solve() with the random ref and session IDs and nil frontendAttrs.

			Session session = Session.create(client, connection);
			Build build = session.createBuild(buildContext, dockerfile);
			build.run();
		}
		return null;
